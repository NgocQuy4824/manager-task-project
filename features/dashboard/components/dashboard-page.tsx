...
export function DashboardPageContent() {
  ...
  const s = data!.data;
  const statusData = s.byStatus.map((r) => ({
    name: (TASK_STATUS_LABELS as Record<string, string>)[r.status] ?? r.status,
    value: r.count,
    key: r.status,
  }));
  const projectData = s.byProject.map((r) => ({
    name: r.name,
    tasks: r.count,
  }));
  ...
  const stats = [
    ...
    },
    {
      label: "Chờ duyệt",
      value:
        s.byStatus.find((x) => x.status === "PENDING_APPROVAL")?.count ?? 0,
      icon: Clock,
      tile: "bg-amber-500/10 text-amber-600",
    },
    {
      label: "Hoàn thành",
      value: s.byStatus.find((x) => x.status === "DONE")?.count ?? 0,
      icon: CircleCheck,
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4 animate-fade-in-up">
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">
            Bức tranh tổng thể về tiến độ công việc của bạn.
          </p>
        </div>
        <Select
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => (
          <Card
            key={stat.label}
            className="animate-fade-in-up border shadow-soft transition-colors hover:border-primary/30"
          >
            <CardContent className="flex items-center gap-4 pt-6">
              <span
                className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${stat.tile}`}
              >
            ...
            <div className="space-y-2">
              {s.recentTasks.map((t) => (
                <Link
                  key={t.id}
                  href={`/tasks/${t.id}`}
                  className="flex items-center justify-between rounded-xl border px-3 py-2 transition-colors hover:border-primary/20 hover:bg-accent/50"
                >
                  <div className="min-w-0">
                  ...
                  <Badge variant="secondary" className="ml-2 shrink-0">
                    {TASK_STATUS_LABELS[t.status as TaskStatusType] ?? t.status}
                  </Badge>
                </Link>
              ))}
            </div>
